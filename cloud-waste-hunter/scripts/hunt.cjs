...
function isDockerFile(filePath) {
    ...
    return basename === 'Dockerfile' || basename === 'docker-compose.yml' || basename === 'docker-compose.yaml';
}

function isKubernetesFile(_filePath, content) {
    return /apiVersion:/.test(content) && /kind:\s*(Deployment|Service|Pod|StatefulSet)/.test(content);
}
...
function checkMissingAutoScaling(content, filePath) {
    ...
    if (/resource\s+"aws_instance"/.test(content) && !/resource\s+"aws_autoscaling_group"/.test(content)) {
        findings.push({
            type: 'missing-autoscaling',
            severity: 'medium',
            file: filePath,
            detail: 'EC2 instance defined without an auto-scaling group. Consider using ASG for better cost efficiency.',
        });
    }
    ...
    if (/kind:\s*Deployment/.test(content) && !/HorizontalPodAutoscaler/.test(content)) {
        findings.push({
            ...
            severity: 'low',
            file: filePath,
            detail: 'Kubernetes Deployment without HorizontalPodAutoscaler reference. Consider adding HPA for dynamic scaling.',
        });
    }
...
function checkUnusedResources(content, filePath) {
    ...
    if (/resource\s+"aws_ebs_volume"/.test(content) && !/aws_volume_attachment/.test(content)) {
        findings.push({
            ...
            severity: 'medium',
            file: filePath,
            detail: 'EBS volume defined without a volume attachment. May result in orphaned storage costs.',
        });
    }
    ...
    if (/resource\s+"aws_nat_gateway"/.test(content)) {
        findings.push({
            ...
            severity: 'low',
            file: filePath,
            detail: 'NAT Gateway detected. These can be expensive; consider NAT instances for dev/test environments.',
        });
    }
    ...
    if (basename === 'Dockerfile') {
        ...
        if (/FROM\s+(ubuntu|debian|centos|amazonlinux)(?::|\s)/m.test(content)) {
            findings.push({
                ...
                severity: 'low',
                file: filePath,
                detail: 'Using a full OS base image. Consider alpine or distroless images to reduce size and cost.',
            });
        }
...
function generateRecommendations(findings) {
    ...
    if (types.has('missing-autoscaling')) {
        recommendations.push('Implement auto-scaling to match capacity with demand and reduce idle costs.');
    }
    ...
    if (types.has('cost-warning')) {
        recommendations.push('Review expensive managed services and consider alternatives for non-production environments.');
    }

    if (recommendations.length === 0) {
        recommendations.push('No significant waste patterns detected. Consider running cloud provider cost analysis tools for deeper insights.');
    }
...
runSkill('cloud-waste-hunter', () => {
    ...
    for (const filePath of allFiles) {
        ...
        const isRelevant = isTerraformFile(filePath)
