...
function checkGDPRCompliance(sources) {
  const concerns = [];
  const hasPersonalData = sources.some(s => s.type === 'database' || s.type === 'browser_storage');
  if (hasPersonalData) {
    concerns.push({ rule: 'Right to be Forgotten', status: 'verify', detail: 'Database/storage access detected - ensure deletion capabilities exist' });
    concerns.push({ rule: 'Data Minimization', status: 'verify', detail: 'Verify only necessary data is collected and stored' });
    concerns.push({ rule: 'Consent', status: 'verify', detail: 'Ensure user consent is obtained before data processing' });
  }
  const apiCalls = sources.filter(s => s.type === 'api_call');
  if (apiCalls.length > 0) concerns.push({ rule: 'Data Transfer', status: 'verify', detail: `${apiCalls.length} external API calls found - verify data transfer compliance` });
  return concerns;
}
...
function buildLineageGraph(sources) {
  ...
  for (const inp of inputs) {
    for (const out of outputs) {
      if (inp.file === out.file) edges.push({ from: `${inp.type}_source`, to: inp.file, via: out.type });
    }
  }
...
runSkill('data-lineage-guardian', () => {
  ...
  const result = {
    directory: targetDir,
    dataSources: sources.slice(0, 50), sourceCount: sources.length,
    dataFlowSummary: { inputs: sources.filter(s => s.flow === 'input').length, outputs: sources.filter(s => s.flow === 'output').length, bidirectional: sources.filter(s => s.flow === 'bidirectional').length },
    gdprConcerns: gdpr, lineageGraph: lineage,
