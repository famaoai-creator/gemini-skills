...
const argv = yargs(hideBin(process.argv))
  .option('input', { alias: 'i', type: 'string', demandOption: true, description: 'Path to JSON with IP portfolio data' })
...
function analyzeIPAssets(data) {
  ...
  return assets.map(asset => {
    ...
    const roi = developmentCost > 0 ? Math.round(((potentialRevenue - annualMaintenance) / developmentCost) * 100) : 0;
    return {
      name: asset.name, type: asset.type || 'software', status: asset.status || 'internal',
      developmentCost, annualMaintenance, potentialRevenue, roi,
...
function designLicensingModels(assets) {
  ...
  for (const asset of assets) {
    if (asset.profitability === 'high' || asset.profitability === 'medium') {
      models.push({ asset: asset.name, models: [
        { type: 'SaaS License', description: 'Monthly/annual subscription access', estimatedRevenue: Math.round(asset.potentialRevenue * 0.7) },
        { type: 'Enterprise License', description: 'Unlimited use for large organizations', estimatedRevenue: Math.round(asset.potentialRevenue * 1.2) },
        { type: 'Open Core', description: 'Free base + paid premium features', estimatedRevenue: Math.round(asset.potentialRevenue * 0.4) },
      ]});
    }
  }
  return models;
...
runSkill('ip-profitability-architect', () => {
  ...
  const result = {
    source: path.basename(resolved), assetCount: assets.length, assets, licensing,
