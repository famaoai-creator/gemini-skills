...
<template>
  <div class="p-4 bg-black border border-gray-800 rounded-xl text-left h-[450px] flex flex-col overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
      <div v-for="report in reports" :key="report.mission_id" class="border-l-2 border-blue-500 pl-4 py-2">
        <div class="flex justify-between items-start mb-2">
          <div>
            <h4 class="text-sm font-black text-white leading-tight">{{ report.topic }}</h4>
            <p class="text-[9px] text-gray-500 font-mono">{{ report.mission_id }} | {{ report.timestamp }}</p>
          </div>
          <span :class="['text-[10px] px-2 py-0.5 rounded font-black', 
        ...
        <div class="grid grid-cols-1 gap-2 mt-3">
          <div v-for="p in report.participants" :key="p.role" class="bg-gray-900 bg-opacity-50 p-2 rounded border border-gray-800">
      ...
      <div v-if="reports.length === 0" class="h-full flex items-center justify-center text-gray-700 italic text-xs animate-pulse">
