...
import { safeExistsSync, safeReadFile, safeReaddir, safeStat } from '@agent/core';
...
function collectModuleSourceInterface(exampleRoot: string, dir: string): { variables: string[]; outputs: string[] } {
  ...
  for (const filePath of listTfFiles(moduleRoot)) {
    const content = String(safeReadFile(filePath, { encoding: 'utf8' }));
...
function collectModuleSourceArtifacts(exampleName: string, exampleRoot: string, allBlocks: TerraformBlock[], moduleSourceDirs: string[], callerBlocksBySource: Map<string, TerraformBlock[]>): any[] {
  if (moduleSourceDirs.length === 0) return [];
  const nodes: any[] = [
    {
      id: `container::module-sources::${slugify(exampleName)}`,
  ...
  for (const dir of moduleSourceDirs) {
    const callers = callerBlocksBySource.get(dir) || [];
    const summary = collectModuleSourceInterface(exampleRoot, dir);
    ...
    nodes.push({
      ...
      provider: 'terraform',
      boundary: 'scope',
      parent: `container::module-sources::${slugify(exampleName)}`,
...
function collectExpandedModuleArtifacts(exampleRoot: string, allBlocks: TerraformBlock[], runtimeBlocks: TerraformBlock[], moduleSourceDirs: string[], networkModel: Map<string, any>): { nodes: any[]; edges: any[] } {
  const nodes: any[] = [];
  const edges: any[] = [];
  for (const callerBlock of runtimeBlocks.filter((item) => item.kind === 'module')) {
    const relSourceDir = resolveModuleSourceDir(callerBlock, exampleRoot);
    if (!relSourceDir || !moduleSourceDirs.includes(relSourceDir)) continue;
    const sourceBlocks = allBlocks.filter((block) => block.dir === relSourceDir && ['resource', 'data'].includes(block.kind)).sort((a, b) => a.id.localeCompare(b.id));
...
function buildEdges(blocks: TerraformBlock[], exampleRoot: string, moduleSourceDirs: string[]): any[] {
  ...
  for (const block of blocks.filter((item) => item.kind === 'module')) {
    const relSourceDir = resolveModuleSourceDir(block, exampleRoot);
    if (!relSourceDir || !moduleSourceDirs.includes(relSourceDir)) continue;
...
export function terraformToArchitectureAdf(exampleRoot: string, options: { title?: string } = {}): any {
  ...
  const allBlocks = [...rootBlocks, ...externalModuleSourceBlocks].filter((block) => ['provider', 'resource', 'data', 'module', 'backend', 'variable', 'output'].includes(block.kind));
  const exampleName = options.title || path.basename(exampleRoot);
  const moduleSourceDirs = collectModuleSourceDirs(exampleRoot, allBlocks);
  const runtimeBlocks = allBlocks.filter((block) => !isWithinModuleSourceDir(block.dir, moduleSourceDirs)).filter((block) => ['provider', 'resource', 'data', 'module', 'backend'].includes(block.kind));
  const callerBlocksBySource = new Map<string, TerraformBlock[]>();
  for (const block of runtimeBlocks.filter((item) => item.kind === 'module')) {
    const relSourceDir = resolveModuleSourceDir(block, exampleRoot);
    if (!relSourceDir) continue;
    const bucket = callerBlocksBySource.get(relSourceDir) || [];
    bucket.push(block);
    callerBlocksBySource.set(relSourceDir, bucket);
  }
  const expandedBlocks = runtimeBlocks
    .filter((item) => item.kind === 'module')
    .flatMap((callerBlock) => {
      const relSourceDir = resolveModuleSourceDir(callerBlock, exampleRoot);
      if (!relSourceDir || !moduleSourceDirs.includes(relSourceDir)) return [];
      const sourceBlocks = allBlocks.filter((block) => block.dir === relSourceDir && ['resource', 'data'].includes(block.kind)).sort((a, b) => a.id.localeCompare(b.id));
      return sourceBlocks.map((sourceBlock) => cloneExpandedModuleBlock(callerBlock, sourceBlock));
    });
  const infraBlocks = [...runtimeBlocks, ...expandedBlocks];
  const networkModel = buildScopeNetworkModel(infraBlocks);
  const containers = collectDirContainers(exampleName, infraBlocks, networkModel);
  const moduleSourceNodes = collectModuleSourceArtifacts(exampleName, exampleRoot, allBlocks, moduleSourceDirs, callerBlocksBySource);
  const expandedModuleArtifacts = collectExpandedModuleArtifacts(exampleRoot, allBlocks, runtimeBlocks, moduleSourceDirs, networkModel);
  const syntheticNetworkNodes = collectSyntheticNetworkNodes(networkModel);
  const syntheticComputeNodes = collectSyntheticComputeNodes(networkModel);
  const nodes = [
    ...containers,
    ...moduleSourceNodes,
    ...expandedModuleArtifacts.nodes,
    ...syntheticNetworkNodes,
    ...syntheticComputeNodes,
    ...runtimeBlocks.map((block) => nodeForBlock(block, networkModel)).filter(Boolean),
  ];
  const edges = [
    ...buildEdges(runtimeBlocks, exampleRoot, moduleSourceDirs),
    ...expandedModuleArtifacts.edges,
  ];

  return {
    version: '1.1.0',
    title: `${exampleName} Terraform Architecture`,
    provider: 'aws',
    render_hints: {
      engine: 'drawio',
      direction: 'LR',
