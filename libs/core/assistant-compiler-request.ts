...
const REQUEST_SCHEMA_PATH = pathResolver.knowledge('public/schemas/assistant-compiler-request.schema.json');
const RESULT_SCHEMA_PATH = pathResolver.knowledge('public/schemas/assistant-compiler-result.schema.json');
...
export function validateAssistantCompilerRequest(value: unknown): { valid: boolean; errors: string[]; value?: AssistantCompilerRequest } {
...
export function validateAssistantCompilerResult(value: unknown): { valid: boolean; errors: string[]; value?: AssistantCompilerResult } {
...
function normalizeExecutionShape(value: unknown): IntentContract['resolution']['execution_shape'] {
  return value === 'direct_reply' || value === 'mission' || value === 'project_bootstrap' ? value : 'task_session';
}

function inferTaskType(text: string, rawResolution?: unknown): string | undefined {
  if (typeof rawResolution === 'object' && rawResolution && typeof (rawResolution as any).task_type === 'string') {
    return (rawResolution as any).task_type;
  }
  if (typeof rawResolution === 'string' && rawResolution.toLowerCase().includes('presentation')) return 'presentation_deck';
  if (text.includes('パワーポイント') || text.toLowerCase().includes('powerpoint') || text.toLowerCase().includes('ppt')) {
...
function normalizeIntentContractFromRaw(
  ...
  const raw = (rawIntentContract && typeof rawIntentContract === 'object') ? rawIntentContract as Record<string, unknown> : {};
  const goalSummary = typeof raw.goal === 'string'
    ? raw.goal
    : typeof raw.goal === 'object' && raw.goal && typeof (raw.goal as any).summary === 'string'
      ? (raw.goal as any).summary
      : request.source_text;
  const requiredInputs = Array.isArray(raw.required_inputs) ? raw.required_inputs.map(String).filter(Boolean) : [];
  const outcomeIds = Array.isArray(raw.outcome_ids)
    ? raw.outcome_ids.map((value) => String(value) === 'presentation_deck' ? 'artifact:pptx' : String(value)).filter(Boolean)
    : (request.source_text.includes('パワーポイント') ? ['artifact:pptx'] : []);
  const approvalValue = typeof raw.approval === 'object' && raw.approval
  ...
  return {
    ...
    intent_id: inferIntentId(request.source_text, raw.intent_id),
    goal: {
      summary: goalSummary,
      success_condition: typeof raw.goal === 'object' && raw.goal && typeof (raw.goal as any).success_condition === 'string'
        ? (raw.goal as any).success_condition
        : `${goalSummary} を governed artifact として成立させる。`,
    },
    resolution: {
      execution_shape: normalizeExecutionShape(typeof raw.resolution === 'object' && raw.resolution ? (raw.resolution as any).execution_shape : raw.resolution),
      task_type: inferTaskType(request.source_text, raw.resolution),
    },
    ...
    clarification_needed: typeof raw.clarification_needed === 'boolean' ? raw.clarification_needed : requiredInputs.length > 0,
...
function normalizeClarificationPacket(
  ...
  const raw = (rawClarificationPacket && typeof rawClarificationPacket === 'object') ? rawClarificationPacket as Record<string, unknown> : {};
  const rawQuestions = Array.isArray(raw.questions) ? raw.questions : [];
  const questions = rawQuestions.length > 0
    ? rawQuestions.map((question, index) => {
        if (typeof question === 'string') {
          return {
            id: contract.required_inputs[index] || `missing_input_${index + 1}`,
            question,
            reason: 'The request cannot be executed safely without this input.',
          };
        }
        return {
          id: typeof (question as any)?.id === 'string' ? (question as any).id : contract.required_inputs[index] || `missing_input_${index + 1}`,
          question: typeof (question as any)?.question === 'string' ? (question as any).question : `Please provide ${contract.required_inputs[index] || 'the missing input'}.`,
          reason: typeof (question as any)?.reason === 'string' ? (question as any).reason : 'The request cannot be executed safely without this input.',
        };
      })
  ...
  if (Array.isArray(rawWorkLoop)) {
    ...
    return {
      ...
      process_design: {
        ...
        operator_checklist: Array.from(new Set([...planOutline, ...normalized.process_design.operator_checklist])),
      },
    };
  }
...
export function normalizeAssistantCompilerResult(
  ...
  const intentContract = normalizeIntentContractFromRaw(request, raw.intent_contract);
  const workLoop = normalizeWorkLoopFromRaw(request, intentContract, raw.work_loop);
  const clarificationPacket = normalizeClarificationPacket(request, intentContract, raw.clarification_packet);
  ...
  const result: AssistantCompilerResult = {
    ...
    source: {
      compiler: 'assistant-subagent',
      provider: typeof raw.source === 'object' && raw.source && typeof (raw.source as any).provider === 'string' ? (raw.source as any).provider : request.delegation.preferred_provider,
      model: typeof raw.source === 'object' && raw.source && typeof (raw.source as any).model === 'string' ? (raw.source as any).model : request.delegation.preferred_model,
    },
  };

  const validation = validateAssistantCompilerResult(result);
  if (!validation.valid) {
    throw new Error(`Invalid normalized assistant compiler result: ${validation.errors.join('; ')}`);
  }
  return result;
}

export function buildAssistantCompilerRequest(input: CreateAssistantCompilerRequestInput): AssistantCompilerRequest {
...
export function createAssistantCompilerRequest(input: CreateAssistantCompilerRequestInput): { request: AssistantCompilerRequest; requestPath: string } {
  const request = buildAssistantCompilerRequest(input);
  const requestPath = writeAssistantCompilerRequest(request);
  return { request, requestPath };
}

export function writeAssistantCompilerResult(result: AssistantCompilerResult, outputPath?: string): string {
  const validation = validateAssistantCompilerResult(result);
  if (!validation.valid) {
