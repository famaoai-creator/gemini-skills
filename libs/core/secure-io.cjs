...
/**
 * Writes an artifact to disk and returns a Hashed Artifact Pointer (HAP) for ADF.
 * @param {string} filePath - Target path
 * @param {string|Buffer} data - Content
 * @param {string} format - Content format (e.g. 'markdown')
 * @returns {import('./shared-business-types').ArtifactPointer}
 */
...
/**
 * Reads an artifact and validates its SHA-256 hash against the pointer.
 * @param {import('./shared-business-types').ArtifactPointer} pointer
 * @returns {string|Buffer}
 * @throws {Error} If hash mismatch or file not found
 */
function readArtifact(pointer) {
  const crypto = require('crypto');
