...
## 3. Agentic Self-Evolution Governance

AI エージェントが自律的に自身のコードやスキルを修正する場合の統制基準。

- **Safe Sandbox**: 自己修正したコードは、まず隔離された環境（Sandbox）で全ユニットテストをパスしなければならない。
- **Peer Review**: 修正内容は別の「監査ロール（Auditor）」が ACE プロトコルに基づき承認し、人間が最終確認を行う。
- **Reversibility**: 全ての自己修正は Git のコミットとして記録され、即座にロールバック可能でなければならない。
