...
3.  **Capability**: `browser-actuator` (ビルド済み)

## 3. 即時実行コマンド (Quick Start)
リポジトリルートから以下のコマンドを実行する。

```bash
pnpm run cli -- run browser-actuator -- --scenario=knowledge/personal/automation/scenarios/intra_ringi.yaml --headless=false
```
...
- 承認した案件名と抽出情報は、capability execution report として不揮発に保存される。
