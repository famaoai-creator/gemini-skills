...
*Before writing code, the AI must understand the existing system.*

1.  **Map the Terrain**: Use `codebase-mapper` to get a high-level view.
    ```bash
    node codebase-mapper/scripts/map.cjs .
    ```
    *Prompt:* "Here is the project structure. Where should I add the new 'UserAuthentication' feature?"

2.  **Understand Data**: Use `schema-inspector` to read DB/API definitions.
    ```bash
    node schema-inspector/scripts/inspect.cjs .
    ```
    *Prompt:* "Based on this schema, write a SQL query to fetch active users."

### Phase 2: Implementation (実装)
*AI generates code based on the context.*
...
1.  **Run Tests**: Use `test-genie` to execute the suite.
    ...
    ```
    *Prompt:* "Here is the test output. Fix the failing test cases."
...
2.  **Self-Review**: Use `local-reviewer` to analyze the diff.
    ...
    ```
    *Prompt:* "Review these changes for security flaws and code style issues. Suggest improvements."
...
1.  **Analyze Logs**: Use `log-analyst` to read runtime errors.
    ...
    ```
    *Prompt:* "Here is the error log. What caused the crash and how do I fix it?"
