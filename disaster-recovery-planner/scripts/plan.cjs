...
function detectInfrastructure(dir) {
  const infra = { services: [], databases: [], storage: [], messaging: [], containerized: false, orchestrated: false };
  ...
  if (exists('docker-compose.yml') || exists('docker-compose.yaml')) {
    try {
      const content = fs.readFileSync(path.join(dir, exists('docker-compose.yml') ? 'docker-compose.yml' : 'docker-compose.yaml'), 'utf8');
  ...
  if (exists('package.json')) {
    try {
      ...
      if (/pg|postgres|mysql2?|sequelize|prisma|typeorm/i.test(allDeps)) infra.databases.push('relational');
...
function assessResilienceGaps(dir, infra) {
  ...
  if (infra.databases.length > 0 && !exists('backup') && !exists('scripts/backup')) {
    gaps.push({ area: 'Backup', severity: 'critical', detail: 'Database detected but no backup scripts found' });
  }
  if (!infra.containerized) {
    gaps.push({ area: 'Portability', severity: 'high', detail: 'No containerization - deployment is environment-dependent' });
  }
  if (!exists('.github/workflows') && !exists('.gitlab-ci.yml')) {
    gaps.push({ area: 'Automation', severity: 'high', detail: 'No CI/CD - manual deployment increases recovery time' });
  }
  if (infra.databases.length > 0 && !infra.orchestrated) {
    gaps.push({ area: 'High Availability', severity: 'medium', detail: 'Database without orchestration - no automatic failover' });
  }
  if (!exists('docs/disaster-recovery.md') && !exists('DR.md')) {
    gaps.push({ area: 'Documentation', severity: 'medium', detail: 'No existing DR documentation' });
  }
  return gaps;
}
...
function generateRunbook(infra, gaps, rto, rpo) {
  ...
  steps.push({ step: stepNum++, phase: 'Detection', action: 'Confirm incident via monitoring alerts and status checks', duration: '5 min', responsible: 'On-call engineer' });
  steps.push({ step: stepNum++, phase: 'Assessment', action: 'Determine scope: which services are affected', duration: '10 min', responsible: 'Incident commander' });
  steps.push({ step: stepNum++, phase: 'Communication', action: 'Notify stakeholders and update status page', duration: '5 min', responsible: 'Incident commander' });

  if (infra.databases.length > 0) {
    steps.push({ step: stepNum++, phase: 'Database Recovery', action: `Restore ${infra.databases.join(', ')} from latest backup (RPO: ${rpo}h)`, duration: '30-60 min', responsible: 'DBA / Backend engineer' });
  }

  if (infra.containerized) {
    steps.push({ step: stepNum++, phase: 'Service Recovery', action: 'Redeploy containers from registry using docker-compose or k8s manifests', duration: '15-30 min', responsible: 'DevOps engineer' });
  } else {
    steps.push({ step: stepNum++, phase: 'Service Recovery', action: 'Provision new infrastructure and deploy application', duration: '60-120 min', responsible: 'DevOps / Backend engineer' });
  }

  steps.push({ step: stepNum++, phase: 'Verification', action: 'Run smoke tests and verify critical user journeys', duration: '15 min', responsible: 'QA engineer' });
  steps.push({ step: stepNum++, phase: 'Post-mortem', action: 'Document timeline, root cause, and corrective actions', duration: '2-4 hours (post-recovery)', responsible: 'Incident commander' });
  ...
  return { steps, estimatedRecoveryMinutes: estimatedRecovery, meetsRTO: estimatedRecovery <= rto * 60 };
}
...
runSkill('disaster-recovery-planner', () => {
  ...
  const result = {
    directory: targetDir,
    rtoHours: argv.rto, rpoHours: argv.rpo,
    infrastructure: infra, resilienceGaps: gaps,
