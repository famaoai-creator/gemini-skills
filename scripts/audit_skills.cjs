#!/usr/bin/env node
/**
 * Skill Quality Audit v3.0 (Prelude Edition)
 * Standards-compliant version (Script Optimization Mission).
 */

const { logger, safeReadFile, pathResolver, requireRole, chalk } = require('./system-prelude.cjs');
const fs = require('fs');
...
function loadIndex() {
  const content = safeReadFile(indexPath, { encoding: 'utf8' });
...
function checkSkill(skillName, skillRelPath) {
  ...
  for (const cDir of codeDirs) {
    if (fs.existsSync(cDir)) {
      ...
      for (const file of files) {
        try {
          const content = safeReadFile(path.join(cDir, file), { encoding: 'utf8' });
  ...
  if (fs.existsSync(skillMd)) {
    try {
      const content = safeReadFile(skillMd, { encoding: 'utf8' });
  ...
  for (const tPath of testFiles) {
    if (fs.existsSync(tPath)) {
      ...
      try {
        const testContent = safeReadFile(tPath, { encoding: 'utf8' });
...
try {
  ...
  if (formatJson) {
    ...
    logger.info(chalk.bold.cyan(`Skill Quality Audit v3.0 - ${results.length} implemented skills`));
