#!/usr/bin/env node
/**
 * System Service Manager v1.0
 * Manages background presence services (Sensors, Daemons).
 */
...
function showStatus() {
  ...
  for (const [id, service] of Object.entries(SERVICES)) {
    ...
    console.log(`  ${id.padEnd(15)} : ${statusStr} ${pidStr}`);
...
switch (action) {
  ...
  case 'status':
    ...
    break;
  default:
    logger.error('Usage: node service_manager.cjs [start|stop|status]');
}
