...
function distill(missionDir) {
  ...
  try {
    ...
    const isSuccess = report.status === 'success' || (logContent && logContent.includes('[SUCCESS]'));
...
`;
    ...
    logger.error(`[Distiller] Error processing ${missionId}: ${err.message}`);
    return null;
  }
}
...
if (require.main === module) {
  try {
    ...
    errorHandler(err, 'Wisdom Distillation Failed');
  }
}
