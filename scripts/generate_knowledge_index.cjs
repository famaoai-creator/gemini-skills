...
try {
  ...
  mdLines.push('');
  mdLines.push(`> Auto-generated by \`scripts/generate_knowledge_index.cjs\` on ${new Date().toISOString().slice(0, 10)}.`);
  ...
  for (const e of entries) {
    ...
    mdLines.push(`| ${e.tier} | ${e.path} | ${safeTitle} | ${e.sizeHuman} | ${e.modifiedDate} | ${staleFlag} |`);
  }
  mdLines.push('');
  ...
  if (staleFiles.length > 0) {
    ...
    mdLines.push('');
    mdLines.push(`The following ${staleFiles.length} file(s) have not been updated in over ${STALENESS_THRESHOLD_DAYS} days and may need review:`);
    mdLines.push('');
