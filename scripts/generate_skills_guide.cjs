...
function generate() {
    ...
    md += 'Total Skills: ' + index.total_skills + ' (Active: ' + index.skills.filter(s => s.status === 'implemented').length + ')\n';
    md += 'Last updated: ' + new Date().toLocaleDateString() + '\n\n';
    md += 'This guide is automatically generated. Efficiency scores are calculated from historical execution data (Latency vs Memory).\n\n';
    ...
    index.skills.sort((a, b) => a.name.localeCompare(b.name)).forEach(skill => {
        ...
        md += '| **' + skill.name + '** | ' + skill.description + ' | ' + scoreStr + ' | ' + timeStr + ' | ' + usage + ' |\n';
    });
