...
const argv = yargs(hideBin(process.argv))
    ...
    .check(parsed => {
        if (!parsed.issue && !parsed.description) throw new Error('Either --issue or --description is required');
        return true;
    })
    .argv;
...
function fetchIssue(issueRef, repo) {
    try {
        ...
        const output = execSync(`gh issue view ${issueRef} ${repoArg} --json title,body,labels,assignees`, {
            encoding: 'utf8',
            timeout: 15000,
            stdio: 'pipe',
        });
