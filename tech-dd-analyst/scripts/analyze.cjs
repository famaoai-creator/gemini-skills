...
function assessTeamMaturity(dir) {
  try {
    const authors = execSync('git log --format="%an" --since="6 months ago" | sort | uniq -c | sort -rn', { cwd: dir, encoding: 'utf8', stdio: ['pipe','pipe','pipe'] });
    const contributors = authors.trim().split('\n').filter(Boolean).map(l => { const m = l.trim().match(/(\d+)\s+(.+)/); return m ? { commits: parseInt(m[1]), name: m[2] } : null; }).filter(Boolean);
    ...
    return { contributors: contributors.length, topContributors: contributors.slice(0,5), busFactor, risk: busFactor <= 1 ? 'critical' : busFactor <= 2 ? 'high' : 'low' };
...
function calculateDDScore(code, team, arch) {
  ...
  if (code.avgFileSize < 300) score += 5; else score -= 5;
  if (team.busFactor >= 3) score += 10; else if (team.busFactor <= 1) score -= 15;
...
runSkill('tech-dd-analyst', () => {
  ...
  const risks = [];
  if (team.risk === 'critical') risks.push({ area: 'Team', severity: 'critical', detail: `Bus factor is ${team.busFactor} - key person dependency` });
  if (arch.testFramework === 'none') risks.push({ area: 'Quality', severity: 'high', detail: 'No test framework detected' });
  if (arch.cicd === 'none') risks.push({ area: 'DevOps', severity: 'high', detail: 'No CI/CD pipeline detected' });
  if (code.avgFileSize > 500) risks.push({ area: 'Maintainability', severity: 'medium', detail: `Average file size ${code.avgFileSize} lines - hard to maintain` });

  const result = {
    directory: targetDir, score, verdict,
    codeQuality: code, teamMaturity: team, architecture: arch, risks,
