...
function detectTechStack(dir) {
  ...
  if (fs.existsSync(path.join(dir, 'requirements.txt')) || fs.existsSync(path.join(dir, 'pyproject.toml'))) stack.languages.push('Python');
...
function generateJobDescription(role, stack) {
  const titles = { engineer: 'Software Engineer', 'senior-engineer': 'Senior Software Engineer', 'tech-lead': 'Technical Lead', devops: 'DevOps Engineer', qa: 'QA Engineer' };
  const requirements = {
    engineer: { experience: '2-4 years', skills: [...stack.languages, ...stack.frameworks, 'Git', 'Testing'], nice: ['CI/CD', 'Cloud services'] },
    'senior-engineer': { experience: '5-8 years', skills: [...stack.languages, ...stack.frameworks, 'System Design', 'Mentoring'], nice: ['Architecture', 'Performance optimization'] },
    'tech-lead': { experience: '7+ years', skills: [...stack.languages, 'Architecture', 'Team leadership', 'Code review', 'Project planning'], nice: ['Cross-team coordination'] },
    devops: { experience: '3-5 years', skills: [...stack.tools, 'Linux', 'Monitoring', 'IaC', 'CI/CD'], nice: ['Kubernetes', 'AWS/GCP/Azure'] },
    qa: { experience: '2-4 years', skills: ['Test automation', 'API testing', ...stack.languages.slice(0, 1), 'Bug tracking'], nice: ['Performance testing', 'Security testing'] },
  };
...
runSkill('talent-requirement-generator', () => {
  ...
  const result = {
    directory: targetDir, role: argv.role,
    detectedTechStack: stack, jobDescription: jd,
    recommendations: [
      `Generated ${jd.title} JD based on detected tech stack`,
      stack.languages.length === 0 ? 'Could not detect languages - provide more context' : `Key skills: ${jd.skills.join(', ')}`,
    ],
  };
