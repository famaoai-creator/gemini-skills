...
export async function GET(req: NextRequest) {
  ...
  if (!isAllowedKnowledgeRefPath(logicalPath)) {
    return NextResponse.json({ error: `knowledge ref is not accessible: ${logicalPath}` }, { status: 403 });
  }
  const resolved = pathResolver.resolve(logicalPath);
  if (!safeExistsSync(resolved)) {
    return NextResponse.json({ error: `knowledge ref not found: ${logicalPath}` }, { status: 404 });
  }
