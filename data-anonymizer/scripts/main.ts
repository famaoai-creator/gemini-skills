...
function anonymize(obj: any): any {
    if (Array.isArray(obj)) {
        ...
        const maskedObj: any = {};
        const SENSITIVE_KEYS = ['email', 'password', 'token', 'secret', 'key', 'apiKey', 'api_key', 'auth'];
        
        for (const [key, value] of Object.entries(obj)) {
            const isSensitive = SENSITIVE_KEYS.some(sk => key.toLowerCase().includes(SENSITIVE_KEYS.find(s => sk.toLowerCase().includes(s.toLowerCase())) || ''));
            // Refined check:
            const matchesSensitive = SENSITIVE_KEYS.some(sk => key.toLowerCase().includes(sk.toLowerCase()));
...
runSkill('data-anonymizer', () => {
    ...
    if (args.out) {
        ...
        return { status: 'success', output: outputPath, message: 'Sensitive data masked successfully.' };
    }
