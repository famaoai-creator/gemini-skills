  ...
  for (const p of myPriorities) {
    ...
    if (p === 'integrity' && role === 'Ecosystem Architect') {
      try {
        ...
        execSync('node scripts/migrated/check_knowledge_integrity.cjs', { stdio: 'ignore', cwd: rootDir });
      } catch (_e) {
        console.log(
    ...
    if (p === 'pii' && role === 'Security Reviewer') {
      try {
        execSync('node scripts/migrated/scan_pii_in_docs.cjs', { stdio: 'ignore', cwd: rootDir });
      } catch (_e) {
        console.log(
    ...
    if (p === 'debt') {
      try {
        ...
        execSync('node scripts/migrated/generate_debt_report.cjs', { stdio: 'inherit', cwd: rootDir });
      } catch (_e) {
        /* ignore */
      }
    }
...
async function finalizeMission(mid) {
  ...
  try {
    ...
    execSync(`node scripts/migrated/archive_missions.cjs`, { stdio: 'inherit', env: { ...process.env, MISSION_ID: mid } });
