...
async function main() {
  ...
  if (syncConfidential.toLowerCase() === 'y') {
    ...
    if (repoUrl) {
      try {
        ...
        execSync(`node scripts/migrated/cli.cjs run sovereign-sync -- init confidential "${repoUrl}"`, {
          stdio: 'inherit',
          cwd: rootDir,
        });
  ...
  try {
    ...
    execSync('node scripts/migrated/generate_skill_index.cjs', { stdio: 'inherit', cwd: rootDir });
  } catch (_e) {
  ...
  try {
    ...
    execSync(`node scripts/migrated/cli.cjs run skill-bundle-packager -- ${missionName} ${skillArgs}`, {
      stdio: 'inherit',
      cwd: rootDir,
    });
