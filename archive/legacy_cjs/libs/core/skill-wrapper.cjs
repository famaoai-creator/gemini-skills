...
function _runBeforeHooks(skillName, args) {
  ...
  const chaosPath = path.join(process.cwd(), 'active/shared/chaos-config.json');
...
function buildOutput(skillName, status, dataOrError, startTime) {
  ...
  const prefs = require('./preference-adapter.cjs');

  const currentRole = fileUtils.getCurrentRole();
  const mid = process.env.MISSION_ID || null;
  ...
  // Deep Data Contract Validation (Skill-specific)
  if (status === 'success' && base.data) {
...
/**
 * Detect if the compiled artifact is older than the source code.
 */
function _checkBuildDrift(skillName) {
  const pathResolver = require('./path-resolver.cjs');
  const skillDir = pathResolver.skillDir(skillName);
