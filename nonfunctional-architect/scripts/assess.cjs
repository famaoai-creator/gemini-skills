...
async function main() {
    ...
    console.log(chalk.gray('This tool will guide you through defining the non-functional requirements for your system.'));
    
    console.log(chalk.yellow('\n【使用条件について】'));
    console.log(chalk.gray('本ツールはIPA「非機能要求グレード2018」を利用しています。'));
    console.log(chalk.gray('ご利用にあたっては、以下のIPA公式サイトの使用条件を必ずご確認ください：'));
    console.log(chalk.blue.underline('https://www.ipa.go.jp/archive/digital/iot-en-ci/jyouryuu/hikinou/ent03-b-1.html\n'));
    ...
    const modelResponse = await prompt({
        ...
        choices: [
            { name: 'low_impact', message: 'Low Impact (Internal/Departmental systems, negligible social impact)' },
            { name: 'mid_impact', message: 'Mid Impact (Standard enterprise systems, limited social impact)' },
            { name: 'high_impact', message: 'High Impact (Critical infrastructure, public systems, large social impact)' }
        ]
    });
