...
export function auditCompliance(dir: string, patterns: string[]): any {
  ...
  for (const p of patterns) {
    ...
    if (p === '.env') {
        ...
        detail = exists ? 'CRITICAL: Raw secrets exposed in project root!' : 'Compliant: No secrets found in source.';
    }

    if (p.includes('kms') || p.includes('vault')) {
        detail = exists ? 'Infrastructure encryption configured.' : 'FISC Requirement: Data at rest encryption missing.';
    }
