...
export function getRecentCommits(dir: string): Commit[] {
  try {
    const output = safeExec('git', ['log', '--oneline', '-20'], { cwd: dir, timeoutMs: 15000 });
    return output
      .split('\\n')
    ...
    throw new Error(`Failed to read git log: ${msg}. Is this a git repository?`);
  }
}

export function getDiffStat(dir: string): ChangedFile[] {
  try {
    const output = safeExec('git', ['diff', '--stat', 'HEAD~1'], { cwd: dir, timeoutMs: 15000 });
...
export function categorizeChanges(changedFiles: ChangedFile[]): Categories {
...
export function generateTitle(commits: Commit[]): string {
...
export function generateDescription(commits: Commit[], changedFiles: ChangedFile[], categories: Categories, repoDir: string): string {
  ...
  if (commits.length > 0) {
    sections.push(
      `This PR includes ${commits.length} commit(s) affecting ${changedFiles.length} file(s).`
    );
  }
...
export function draftPR(repoDir: string): PRDraft {
  ...
  const description = generateDescription(commits, changedFiles, categories, repoDir);
