...
export function auditDataRoom(dir: string, stage: Stage) {
  ...
  for (const category of checklist.required) {
    ...
    results.push({
      ...
      completion: Math.round((categoryResults.filter((r) => r.status === 'found').length / categoryResults.length) * 100),
    });
  }
...
export function assessRisks(audit: { results: CategoryResult[]; completionPercent: number }, stage: Stage): RiskAssessment[] {
  ...
  for (const cat of audit.results) {
    ...
    if (missing.length > 0) {
      const severity: RiskAssessment['severity'] = (cat.category === 'Legal' || cat.category === 'Financial') ? 'high' : 'medium';
      risks.push({
