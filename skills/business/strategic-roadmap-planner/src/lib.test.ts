...
import fs from 'fs';
import { analyzeCodeComplexity, detectTechDebt, checkInfrastructure, generateRoadmap } from './lib.js';
...
describe('strategic-roadmap-planner lib', () => {
  ...
  it('should detect tech debt from comments', () => {
    ...
    vi.spyOn(fs, 'readFileSync').mockReturnValue('// TODO: fix this\n// HACK: temporary fix\n// FIXME: bug here');
  ...
  it('should check infrastructure state', () => {
    vi.spyOn(fs, 'existsSync').mockImplementation((p) => {
        if (typeof p === 'string' && (p.includes('tsconfig.json') || p.includes('README.md'))) return true;
        return false;
    });
  ...
  it('should generate strategic roadmap based on inputs', () => {
    ...
    const infra = { hasCICD: false, hasTests: false, hasLinting: false, hasTypeChecking: true, hasDocumentation: true, hasContainerization: false };
  ...
  it('should elevate priority when velocity is stalled despite low debt', () => {
    ...
    expect(result.priorities.some(p => p.priority === 'critical' && p.action.includes('Stalled velocity'))).toBe(true);
  });
});
