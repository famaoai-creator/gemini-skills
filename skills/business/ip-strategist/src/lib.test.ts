...
describe('ip-strategist lib', () => {
  ...
  it('should detect license protection and risk', () => {
    ...
    const readSpy = vi.spyOn(fs, 'readFileSync').mockReturnValue('This project is licensed under the MIT License.');
  ...
  it('should process full IP strategy from codebase', () => {
    const existsSpy = vi.spyOn(fs, 'existsSync').mockImplementation((p) => typeof p === 'string' && p.includes('LICENSE'));
    ...
    const getAllFilesSpy = vi.spyOn(fsUtils, 'getAllFiles').mockReturnValue(['/test/algo.ts', '/test/LICENSE']);
  ...
  it('should detect advanced AI terms as Algorithm/Model', () => {
    const readSpy = vi.spyOn(fs, 'readFileSync').mockReturnValue('A new transformer model for inference.');
