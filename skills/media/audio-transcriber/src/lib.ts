...
export async function transcribeAudio(filePath: string, options: TranscribeOptions): Promise<TranscribeResult> {
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      const isRetryable = err.code === 'ECONNABORTED' || (err.response && (err.response.status === 429 || err.response.status >= 500));
