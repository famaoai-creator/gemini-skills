...
function detectTechDebt(dir) {
  ...
  return {
    ...
    hotspots: indicators.sort((a, b) => (b.todos + b.hacks + b.fixmes) - (a.todos + a.hacks + a.fixmes)).slice(0, 10),
  };
}

function getRecentVelocity(dir) {
  try {
    const weeks4 = execSync('git log --oneline --since="4 weeks ago" --no-merges', { cwd: dir, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
    const weeks1 = execSync('git log --oneline --since="1 week ago" --no-merges', { cwd: dir, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
...
function checkInfrastructure(dir) {
  const checks = {
    hasCICD: false, hasTests: false, hasLinting: false, hasTypeChecking: false,
    hasDocumentation: false, hasContainerization: false,
  };
  ...
  checks.hasLinting = exists('.eslintrc.js') || exists('.eslintrc.json') || exists('eslint.config.mjs') || exists('.pylintrc');
  checks.hasTypeChecking = exists('tsconfig.json') || exists('mypy.ini') || exists('pyrightconfig.json');
...
function generateRoadmap(complexity, debt, velocity, infra, months) {
  ...
  // Priorities
  if (debt.debtScore > 50) priorities.push({ priority: 'critical', action: `Tech debt score ${debt.debtScore}/100 - allocate 30% capacity to debt reduction` });
  if (velocity.avgPerWeek < 5) priorities.push({ priority: 'high', action: 'Low velocity detected - review blockers and process efficiency' });
  if (complexity.avgFileSize > 200) priorities.push({ priority: 'medium', action: `Average file size ${complexity.avgFileSize} lines - consider modularization` });
...
runSkill('strategic-roadmap-planner', () => {
  ...
  const result = {
    ...
    codeAnalysis: { totalFiles: complexity.totalFiles, totalLines: complexity.totalLines, avgFileSize: complexity.avgFileSize, languages: complexity.languages, largeFiles: complexity.largeFiles.slice(0, 5) },
    techDebt: debt,
    velocity,
